...
class VQADataset(torch.utils.data.Dataset):
    def __init__(self, df_path, image_dir, transform=None, answer=True):
        ...
        if self.answer:
            # 回答辞書にデータを追加しておく。
            answer_copus = pandas.read_csv("data/class_mapping.csv")
    ...
    def __getitem__(self, idx):
        ...
        # 質問に対して、前処理を追加（改良点その１）
        processed_text = process_text(self.df["question"][idx])
        
        # 質問を、BERTNのtokenizer等を利用して分散表現に変換
        # モデルの読み込み
        with torch.no_grad():
            # model = SentenceTransformer('model/sentence-transformers/all-mpnet-base-v2')
            model = SentenceTransformer('model/sentence-transformers/all-MiniLM-L6-v2')
            question = model.encode(processed_text, convert_to_tensor=True)
...
def main():
    ...
    transform = transforms.Compose([
        ...
        transforms.RandomHorizontalFlip(p=0.5), # horizontally flipping
    ...
    # 質問のサイズを768次元に変更
    model = VQAModel(vocab_size=384, n_answer=len(train_dataset.answer2idx)).to(device)
